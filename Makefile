...
# Install/Uninstall make script for clang-utilities/build-clang-makefile
...
#
SCRIPT_NAME := build-clang-makefile
...
man: ## Builds manual pages via `help2man` command
	if [[ -d "$(MAN__SOURCE_DIR)" ]]; then
		help2man "$(SCRIPT__SOURCE_PATH)"\
			--help-option="--help --name project-name"\
			--output="$(call path_append, $(MAN__SOURCE_DIR), $(NAME)).1"\
			--no-info
	fi
...
completion: ## Builds tab completion Bash configuration file for build-clang-makefile
	"help-to-complete" --executable "$(ROOT_DIRECTORY_PATH)/build-clang-makefile" --completion-dir "$(ROOT_DIRECTORY_PATH)/bash-completion" --clobber

link-completion: SHELL := /bin/bash
link-completion: ## Links tab completion  Bash configuration file for build-clang-makefile
